(function () {
  ...
  function init() {
    ...
    function getUserIdInfo(user) {
      const idValue = user.studentId || user.facultyId || user.campusId;
      if (!idValue) return "";
      const label = user.studentId
        ? "Student ID"
        : user.facultyId
        ? "Faculty ID"
        : "Campus ID";
      return `<p class="user-meta">${label}: ${idValue}</p>`;
    }
    ...
    function renderUsers() {
      ...
      const filteredUsers = usersToShow.filter((user) => {
        ...
        const userIdValue = user.studentId || user.facultyId || user.campusId || "";
        return [
          user.fullName,
          user.email,
          user.username,
